...
export default function GameScreen({ launch }: { launch: GameLaunch }): ReactElement {
  ...
  const room = session.phoneRoomCode;
  const relayStatus: RelayStatus = session.phoneStatus;
  ...
  useEffect(() => subscribeToSession((event) => {
    if (event.type === "phoneSample") {
      ...
      gameRef.current?.setControllerState(event.sample);
    } else if (event.type === "remotePose") {
      gameRef.current?.setRemoteSwordNetworkPose(event.pose);
    } else if (event.type === "combatIntent") {
      gameRef.current?.setRemoteCombatIntent(event.intent);
    } else if (event.type === "combatOutcome") {
      applyCombatOutcomeToUi(event.outcome);
  ...
  const newRoom = () => {
    session.regeneratePhoneRoom();
    setCopyLabel("Copy controller link");
  };

  const joinMatch = () => {
    void session.joinMatch(matchCode);
  };
  ...
  return (
    <main className="app-shell">
      <button className="game-menu-back" type="button" onClick={returnToMenu}>
        MENU
      </button>
      <OnEscape onEscape={returnToMenu} />
      ...
      <aside className="hud inspector-panel" aria-label="Motion lab controls">
        ...
        <div className="section-block pairing-block">
          ...
          <div className="diagnostic-row"><span>URI</span><strong className="mono-value">{session.endpoint.uri}</strong></div>
          <div className="diagnostic-row"><span>DB</span><strong className="mono-value">{session.endpoint.database}</strong></div>
