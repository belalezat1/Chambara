...
This checkout combines the gameplay from `core` with the menus and lobby flow from `side`. In the current workspace, use **`C:\merger\core`** as the project root and **`C:\merger\core\game`** to run the frontend.

## Requirements

- Node.js **22.12 or newer**, with a working npm installation.
- A desktop browser; Chrome is used by the included browser smoke test.
...
On this Windows workspace, you can first run `cd C:\merger\core\game`. If npm reports a missing `npm-cli.js`, repair the Node/npm installation or use an installed pnpm (`pnpm install`, then `pnpm run dev`).
...
For the browser smoke test, keep the dev server running and run:

```sh
npm run test:merged-ui
```

The test creates two isolated browser clients, makes and joins a lobby, checks that both reach the game screen, and checks Settings and Practice navigation. It uses whichever backend the running frontend is configured to use and creates a temporary test room there. It does not test physical phone sensors or a complete played duel.

The script defaults to `http://127.0.0.1:5173` and Chrome at `C:\Program Files\Google\Chrome\Application\chrome.exe`. Override either with environment variables. PowerShell example:

```powershell
$env:CHAMBARA_URL = 'http://127.0.0.1:5174'
$env:CHAMBARA_BROWSER = 'C:\path\to\chrome.exe'
npm run test:merged-ui
```
...
```text
game/
  src/App.tsx             Screen routing and application shell
  src/ui/                 Menus, lobby, settings, and shared match session
  src/game/               Babylon runtime, combat, input, and networking
  src/components/         Phone controller screen
  src/module_bindings/    Generated SpacetimeDB client bindings
  public/                 Served character assets, UI art, and audio
  scripts/                Tunnel helpers and browser verification
  vite.config.ts          Dev server and /motion-ws phone relay
spacetimedb/
  spacetimedb/src/        Backend tables and reducers
assets/                   Source character assets
```
...
- **Browser test cannot launch Chrome:** set `CHAMBARA_BROWSER` to an installed Chrome/Chromium executable.

See [game/CREDITS.md](game/CREDITS.md) for asset credits. This README describes the current merged setup; older setup notes may still refer to the previous database or an obsolete `npm run tunnel` command.
